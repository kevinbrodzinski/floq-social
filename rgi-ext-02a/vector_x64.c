...
int main(void){
  ...
  int fma = __builtin_cpu_supports("fma") != 0;
  int avx512 = __builtin_cpu_supports("avx512f") != 0;
  if(!avx2 || !fma){ fprintf(stderr,"AVX2/FMA unavailable\n"); return 4; }
  ...
  for(int r=0;r<reps;r++){
    for(size_t i=0;i<n;i+=8){
      __m256 x=_mm256_load_ps(a+i), y=_mm256_load_ps(b+i);
      _mm256_store_ps(c+i,_mm256_fmadd_ps(x,y,half));
    }
  }
  uint64_t t1=ns();
  ...
  printf("{\"architecture\":\"x86_64\",\"kernel\":\"AVX2_FMA\",\"avx2\":true,\"fma\":true,\"avx512f\":%s,\"elements\":%zu,\"repetitions\":%d,\"elapsed_ns\":%llu,\"effective_bytes_per_second\":%.3f,\"checksum\":%.9f}\n",avx512?"true":"false",n,reps,(unsigned long long)(t1-t0),bytes/sec,checksum);
